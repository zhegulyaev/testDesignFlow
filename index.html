...
<head>
  ...
  <title>DesignFlow — Трекер целей (Fixed)</title>
  ...
  <div class="header-wrapper">
    <div class="header">
      <div class="logo-container">
        ...
        <div class="logo-text">
          <h1>DesignFlow <span>v0.4</span></h1>
          <p>Трекер целей</p>
        </div>
      </div>
      <div class="header-actions">
        <div class="authbar" style="display:flex;gap:10px;align-items:center;">
          <button id="btn-login" class="btn primary" type="button"><i class="fa-brands fa-google"></i> <span class="desktop-only">Войти</span></button>
          <button id="btn-logout" class="btn secondary" type="button" style="display:none;"><i class="fa-solid fa-right-from-bracket"></i></button>
          <span id="auth-status" style="opacity:.75;font-size:13px; display: none;"></span>
        </div>
  ...
  <div class="warning-wrapper">
    <div class="warning"> 
        <div class="warning-content">
            ...
            <div>
                <strong class="warning-red-text">Важно:</strong> <span id="sync-warning">Данные хранятся только в вашем браузере. Войдите, чтобы сохранить их в облаке.</span><br>
                Обязательно скачивайте резервную копию перед очисткой кэша.
            </div>
        </div>
...
(() => {
  ...
  // --- SAFE ID GENERATOR (FIX for file:// protocol) ---
  function generateId() {
      // Пытаемся использовать crypto (если есть)
      if (typeof crypto !== 'undefined' && crypto.randomUUID) {
          return crypto.randomUUID();
      }
      // Фоллбэк для старых браузеров или небезопасного контекста (local files)
  ...
  // --- MAPPING FUNCTIONS (Supabase snake_case <-> App camelCase) ---
  function toDbFormat(goal, userId) {
      return {
          ...
          notes: goal.notes,
          is_done: goal.status === 'done',
          is_trash: goal.status === 'trash',
  ...
  function toLocalFormat(dbGoal) {
      ...
      return {
          ...
          notes: dbGoal.notes,
          status: status,
          isFavorite: dbGoal.is_favorite,
          isPinned: dbGoal.is_pinned,
          history: dbGoal.history || [],
  ...
  async function syncGoalToCloud(goal) {
      if (!user || !sb) return;
      try {
          const dbData = toDbFormat(goal, user.id);
          const { error } = await sb.from('goals').upsert(dbData);
          if (error) throw error;
      } catch (e) {
          console.error("Cloud Sync Error:", e);
          showToast('Ошибка синхронизации с облаком');
      }
  }
  ...
  async function deleteGoalFromCloud(id) {
      if (!user || !sb) return;
      try {
          const { error } = await sb.from('goals').delete().eq('id', id).eq('user_id', user.id);
          if (error) throw error;
      } catch (e) {
  ...
  function updateAuthUI() {
    ...
    if (user) {
      ...
      warn.innerHTML = `<span style="color:var(--green)">Облачная синхронизация активна.</span>`;
    } else {
      btnL.style.display = 'block';
      btnO.style.display = 'none';
      status.style.display = 'none';
      warn.innerHTML = `Данные хранятся только в вашем браузере. Войдите, чтобы сохранить их в облаке.`;
    }
  }

  document.getElementById('btn-login').onclick = async () => {
    if (!sb) return alert("Ключи Supabase не настроены!");
  ...
  function safeParseNum(v) {
    ...
    if (str.endsWith('к') || str.endsWith('k')) {
        str = str.slice(0, -1);
        multiplier = 1000;
    } else if (str.endsWith('м') || str.endsWith('m')) {
        str = str.slice(0, -1);
        multiplier = 1000000;
    }
  ...
  function save(goalToSync = null) {
    // 1. Всегда сохраняем локально (кэш/бэкап)
    try {
        localStorage.setItem(K, JSON.stringify({data: {goals: DATA.goals}}));
    } catch(e) {
        showToast('Ошибка сохранения локально!');
        console.error("LS Save Error:", e);
    }

    // 2. Если залогинены - отправляем в облако
    if (user && goalToSync) {
        syncGoalToCloud(goalToSync);
    }
  }

  async function load() {
    try {
      // 1. По умолчанию пробуем грузить локально
      const raw = localStorage.getItem(K);
      if (raw) {
        const parsed = JSON.parse(raw);
        DATA.goals = Array.isArray(parsed.data?.goals) ? parsed.data.goals : [];
        DATA.goals.forEach(g => { if (!g.history) g.history = []; });
      }

      // 2. Если пользователь залогинен, загружаем из Supabase
      if (user && sb) {
          ...
          if (data && data.length > 0) {
              ...
              // ВАЖНОЕ ИСПРАВЛЕНИЕ:
              // Если в облаке пусто (0 записей), а локально что-то есть - НЕ стираем локальные данные.
              // Это защищает пользователя, который залогинился, но у него пустая БД.
              if (DATA.goals.length > 0) {
                  showToast('Локальные данные сохранены (Облако пусто)');
              }
          }
      }
        ...
        if (!DATA.goals.length) showToast('Ошибка загрузки данных.');
    }
  }
  ...
  if(btnMultiFinalDelete) {
      btnMultiFinalDelete.onclick = () => {
          ...
          openConfirmModal(`Удалить ${selected.length} целей безвозвратно?`, () => {
              ...
              // Sync Deletion
              idsToDelete.forEach(id => deleteGoalFromCloud(id));
  ...
  function reorderGoals(draggedId, targetId) {
      ...
      if (draggedIndex > -1 && targetIndex > -1) {
          ...
          save(); // Order only saves locally for now unless we add an 'order' field to DB
          render();
      }
  }
        ...
        btn.onclick = (e) => {
            ...
            if (act === 'del-final') {
                openConfirmModal(`Удалить цель "${g.title}" безвозвратно?`, () => {
                    ...
                    save(); // Local save
                    deleteGoalFromCloud(goalId); // Cloud delete
                    render();
                    showToast('Удалено');
                }, true);
            }
  ...
  document.getElementById('btn-demo').onclick = () => {
    ...
    demoGoals.forEach(rnd => {
        ...
        if (current > 0) {
            g.history.push({ id: generateId(), date: nowISO(), amount: current, type: 'set', comment: 'Начальный баланс' });
        }
        DATA.goals.push(g);
        save(g); // Sync
    });
  ...
  document.getElementById('file-import').onchange = e => {
    ...
    reader.onload = () => {
        try {
            ...
            // Обработка разных форматов импорта
            let newGoals = [];
            if (Array.isArray(json)) newGoals = json;
            else if (json.data && Array.isArray(json.data.goals)) newGoals = json.data.goals;
            
            // Если формат был другой, пробуем привести
            newGoals = newGoals.map(g => {
                if (!g.id) g.id = generateId();
                return g;
            });

            openConfirmModal('Загрузить данные? Текущие будут заменены!', async () => {
                // Если облако активно, нужно будет загрузить туда
                DATA.goals = newGoals;
                
                if (user) {
                     showToast('Импорт в облако может занять время...');
                     for (const g of DATA.goals) {
                         await syncGoalToCloud(g);
                     }
                }
